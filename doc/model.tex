...
=
\begin{bmatrix}
1-\frac{\rho \lambda_T}{2} & \delta^{-1} \\
\frac{\rho \lambda_T}{2} & (1-\delta^{-1})
\end{bmatrix}
\begin{bmatrix}
P(s_{k}=c|s_1) \\
P(s_{k}=r|s_1)
...
recombination.  Note that the parameters $\rho$ and $\delta$ do not
\emph{precisely} correspond to the recombination rate and tract
length, although they do provide a good approximation when
$\rho\delta$ is small.
...
\begin{equation}
P(s_0=c|T,\rho,\delta)=\frac{1}{\frac{\rho\lambda_T\delta}{2}+1}=1-P(s_0=r|T,\rho,\delta)
\end{equation}
...
\begin{tabular}{|cl|}
  ...
  $\rho$ & Recombination rate (events per unit time) parameter \\
  $\delta$ & Average converted tract length parameter \\
