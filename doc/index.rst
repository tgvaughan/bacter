...
example `bacter_tutorial.xml`), and press the Save button to produce the BEAST
input XML.
...
1. The **log** file, which ends in the extension `.log` and contains sampled parameter values,
2. The **tree** file, which ends in the extension `.trees` and contains sampled ARGs.
...
[IcyTree](http://tgvaughan.github.io/icytree) phylogenetic tree and network
viewer. Betware that the viewer requires an up-to-date version of Firefox or
Chrome to function correctly.

To use the viewer, simply open the
[IcyTree](http://tgvaughan.github.io/icytree) web page in a browser window,
...
period (`,` and `.`) keys to step through the file one ARG at a time or the `<`
and `>` keys to step in increments of 10%.  Navigation can also be performed by
clicking on the buttons in the lower-left corner of the window with your mouse.
Further information about using IcyTree can be found by selecting items listed
under the Help menu.
...
analysis is selected and choose `summary.tree` as the output file name.  Leave
